...
/*   Updated: 2025/02/12 17:31:03 by lhaas            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_swaps(t_stack *st_a, t_stack *st_b, int pivot)
{
	if (st_b->size > 3
		&& st_b->arr[st_b->head].rank < st_b->arr[(st_b->head + 1)
			% st_b->capacity].rank && st_b->arr[st_b->head].rank >= pivot
		&& st_a->size > 3 && st_a->arr[st_a->head].rank
		> st_a->arr[(st_a->head + 1) % st_a->capacity].rank
		& st_a->arr[st_a->head].rank <= pivot)
	{
		swap(st_b);
		swap(st_a);
		ft_printf("ss\n");
...
void	handle_moves(t_stack *stack_a, t_stack *stack_b, int pivot, int *moves)
{
	int	next_index;
...
void	partition_b(t_stack *stack_a, t_stack *stack_b, int pivot)
{
	...
	while (i < original_size)
	{
		...
		{
			handle_swaps(stack_a, stack_b, pivot);
			handle_moves(stack_a, stack_b, pivot, &moves);
		}
